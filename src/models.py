...
from sqlalchemy import DateTime, MetaData, UniqueConstraint, distinct
...
class Dataframe(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(
        db.String(100), unique=True, nullable=False, info={"label": "Название"}
    )
    description = db.Column(db.String(150), info={"label": "Описание"})
    # fk must be table name not model, use_alter to prevent unresolved cycles between
    # check and dataframe
    check_id = db.Column(
        db.Integer, db.ForeignKey("checks.id", ondelete="set null", use_alter=True)
    )

    active_check = db.relationship(
        "Check",
    ...
    urls = db.relationship(
        "Url", back_populates="dataframe", lazy="select", cascade="all, delete-orphan"
    )
    ...
    clusters = db.relationship(
        "Cluster",
        secondary="dataframes_clusters",
        back_populates="dataframes",
        lazy="select",
    )
    properties = db.relationship(
        "DataframeProperty", back_populates="dataframe", lazy="select"
    )
...
class Url(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    dataframe_id = db.Column(
        db.Integer, db.ForeignKey(Dataframe.id, ondelete="cascade"), nullable=False
    )
    ...
    checks = db.relationship(
        "UrlCheck", back_populates="url", lazy="select", cascade="all, delete-orphan"
    )
    dataframe = db.relationship(
        Dataframe, back_populates="urls", lazy="select", passive_deletes=True
    )
...
class Check(db.Model):
    """Check is used for url parsing - connects urls data with a dataframe"""
    ...
    selectors = db.Column(
        db.Text, info={"label": "Селекторы для извлечения данных"}
    )  # TODO delete when use Property
    # for different dbs
    ...
    urls = db.relationship(
        "UrlCheck", back_populates="check", lazy="select", cascade="all, delete-orphan"
    )
    dataframe = db.relationship(
        Dataframe, back_populates="checks", foreign_keys=[dataframe_id], lazy="select"
    )
    ...
    def handled_url_count(self):
        return (
            db.session.query(distinct(UrlProperty.url_id))
            .filter_by(check_id=self.id)
            .count()
        )
...
class UrlCheck(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    url_id = db.Column(
        db.Integer, db.ForeignKey(Url.id, ondelete="cascade"), nullable=False
    )
    check_id = db.Column(
        db.Integer, db.ForeignKey(Check.id, ondelete="cascade"), nullable=False
    )
...
class Property(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(
        db.String(50), unique=True, nullable=False, info={"label": "Название"}
    )
    code = db.Column(db.String(50), unique=True, nullable=False)

    dataframes = db.relationship(
        "DataframeProperty", back_populates="property", lazy="select"
    )

    def __str__(self):
        return self.name

    # dataframes = db.relationship(DataFrame, secondary='DataFrameProperty',
    # back_populates='properties', lazy='select')
...
class DataframeProperty(db.Model):
    ...
    dataframe = db.relationship(
        Dataframe, back_populates="properties", lazy="select", passive_deletes=True
    )
    property = db.relationship(
        Property, back_populates="dataframes", lazy="select", passive_deletes=True
    )
...
class UrlProperty(db.Model):
    ...
    UniqueConstraint(
        url_id, check_id, property_id, name="urls_properties_url_check_property_key"
    )
...
class DataframeCluster(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    dataframe_id = db.Column(
        db.Integer, db.ForeignKey(Dataframe.id, ondelete="cascade")
    )
    cluster_id = db.Column(db.Integer, db.ForeignKey(Cluster.id, ondelete="cascade"))

    UniqueConstraint(
        dataframe_id, cluster_id, name="dataframes_clusters_dataframe_id_cluster_id_key"
    )
