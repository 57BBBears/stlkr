...
#     )
#     df = Resource.query.order_by(Resource.id).paginate(per_page=per_page)
...
#     df = Resource.query.get_or_404(pk)
...
#         )
#         .group_by(Url.id, Url.address, UrlCheck.status)
...
#
#     if form.validate_on_submit():
#         df = Resource(name=form.name.data, description=form.description.data)
#         db.session.add(df)
#         db.session.commit()
#
#         if form.urls.data:
...
#
#     urls = (
#         db.session.query(Url.id, Url.address, UrlCheck.status)
#         .outerjoin(Url.checks.and_(UrlCheck.check_id == check_id))
...
#     # TODO change to check.is_checking - get current job status at a queue by job_id ?
#     cur_check = (
...
#     :param selectors:
#     :param properties:
#     :return: dict with url as keys and dict of [Selector name, Selector value]
#     as values
#     """
#     urls_data = {}
#
...
#         (Cluster.id != pk) & ((Cluster.parent_id != pk) |
#         (Cluster.parent_id.is_(None)))
#     ).all()
...
#     query = Resource.query.with_entities(Resource.id, Resource.name)
...
#                             Resource.query.with_entities(Resource.id).filter(
#                                 Resource.name.in_(dataframes_for_del)
#                             )
#                         ),
#                     )
#                 )
#             if dataframes_for_add:
#                 dataframes_for_add_ids = Resource.query.with_entities(
#                     Resource.id
#                 ).filter(Resource.name.in_(dataframes_for_add))
#                 db.session.execute(
...
#
#     return render_template("core/cluster_add.html", title="Добавить кластер",
#     form=form)
#
#
