...
def category(slug):
    ...
    cluster_active_checks = (
        db.session.query(Dataframe.check_id.label("check_id"))
        .join(DataframeCluster)
        .where(
            DataframeCluster.c.cluster_id == cluster.id, Dataframe.check_id.isnot(None)
        )
        .subquery()
    )
...
def detail(pk):
    active_check = (
        ...
        .join(Dataframe, Dataframe.check_id == Check.id)
